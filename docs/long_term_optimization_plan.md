...
对比优秀数学建模论文样本与当前真实题目输出，当前系统已经能形成完整论文骨架、模型假设、公式推导、部分图表和导出产物，但距离优秀论文仍有明显差距。优秀论文的核心特征不是单纯篇幅更长，而是每个子问题都形成“问题分析 -> 模型选择 -> 参数求解 -> 结果表格 -> 图形解释 -> 检验/评价”的闭环。

### 10.1 当前主要差距

1. 数值求解闭环不足：当前输出存在大量 `blocked` / `partial coverage`，能写出模型框架，但不能稳定给出题目要求的最终参数、表格填充值和关键结论。优秀论文会在摘要和各问题小节直接给出核心数值结果。
2. 模型选择理由不足：当前论文常直接给出分段函数、最小二乘、优化目标等，但缺少“为什么这样选、其它模型为什么不用、该简化会带来什么误差”的论证。优秀论文会先做问题分析，再用数据、物理机制或统计检验支撑模型选择。
3. 结果表格体系不足：当前表格多用于状态说明或中间摘要，缺少题目要求的最终表格、参数表、对比表、显著性表、敏感性表。优秀论文会把关键结果集中成表，并让正文围绕表格解释。
4. 图表类型偏结果诊断：当前主要是拟合曲线、残差图，缺少坐标系示意图、模型流程图、变量关系图、敏感性曲线、参数对比图等辅助理解图。优秀论文会用图说明模型结构、受力/流程/变量关系，而不仅展示结果曲线。
5. 检验与稳健性不足：当前有误差分析段落，但缺少可执行的显著性检验、残差随机性检验、灵敏度分析、参数置信区间、替代模型对比。优秀论文会用检验结果反过来支持模型可信度。
6. 摘要结果密度不足：当前摘要偏“做了什么”和“受限说明”，优秀论文摘要会逐问题交代方法和关键结果，让评阅者第一屏看到贡献和数值答案。
7. 附录与可复现实验不足：当前虽有 notebook 和 artifacts，但论文附录没有稳定呈现核心代码片段、参数求解过程、数据预处理说明、表格填充说明。优秀论文通常把程序、拟合函数、主要数据表放入附录。

### 10.2 长线新增优化任务

1. 建立 `ResultCompletenessGate`：按题目子问题检查是否产出最终答案、参数表、题目指定表格；只有模型框架而无最终数值时，不能标记为完整通过，只能进入 partial delivery。
2. 建立 `AnswerTablePlannerSkill`：从题目和 solve_spec 中抽取“必须填写/必须回答”的表格与答案槽位，生成 `answer_table_plan.json`，并要求 Solver/Reviewer 明确填充或给出阻断原因。
3. 建立 `ModelChoiceRationaleSkill`：为每个子问题生成模型选择理由、替代模型、简化假设风险，供 Writer 使用，避免论文只罗列公式。
4. 扩展 `VisualizationPlannerSkill`：除结果图外，默认为复杂建模题生成至少一张模型结构/解题流程/变量关系图；为优化题生成收敛曲线和敏感性曲线；为统计题生成显著性/残差诊断图。
5. 建立 `ValidationPlanSkill`：根据 result_type 自动要求对应检验，包括回归显著性、残差随机性、分类混淆矩阵、优化收敛性、敏感性分析、参数置信区间。
6. 建立 `AbstractResultContract`：摘要必须逐问题包含“方法 + 核心数值/结论 + 可信度状态”。若结果被阻断，摘要必须明确说明哪个问题缺少最终答案，不能用泛化语句掩盖。
7. 建立 `PaperScoreDiagnostic`：在 `verified_output_bundle.json` 中增加论文质量评分维度：answer completeness、model rationale、table coverage、figure diversity、validation strength、abstract density、appendix reproducibility。
8. 建立优秀论文回归 fixture：选择至少 3 类样本题（物理建模、优化排样、金融/统计评价），用固定输出指标对比优秀论文结构，避免只按单一交通流题优化。

### 10.3 近期优先级

1. 优先补 `AnswerTablePlannerSkill` 和 `ResultCompletenessGate`，因为当前最大短板是“有论文但没有最终答案闭环”。
2. 其次补 `ValidationPlanSkill`，让结果不是只展示曲线，而是能被检验支持。
3. 再补图表多样性协议，确保每篇复杂建模论文至少同时包含结果图、模型辅助说明图、检验/敏感性图。
4. 最后优化 Writer 摘要和结论模板，让论文表达接近优秀论文的“逐问题、有数字、有判断、有局限”风格。
