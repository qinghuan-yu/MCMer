...
MCMer 是一个面向数学建模场景的全栈系统。当前主线聚焦高可信建模指导：从题目与附件中提取可计算问题，明确观测量、未知量和信息缺口，执行可复核的数值计算，并把参数来源、模型边界、阻断原因和结果证据组织成可下载的指导工作区。

系统不承诺为任意题目自动生成完整答案。数据不足、目标不可辨识或计算能力尚未覆盖时，主线会保留已完成部分，并明确说明缺少什么以及如何恢复，而不是用经验常数或占位公式补齐结果。

## 分支定位

- `main`：原 `guider` 主线。面向高可信数学建模指导，正在从整题求解模板迁移到强类型 Model IR、通用数学算子、独立验证和证据驱动 Guidance。
- `agent`：原 `main` 主线。保留多 Agent 论文生产、论文润色、DOCX 导出和历史项目工作流，便于继续维护原有产品形态。

`main` 是当前默认开发方向。`agent` 不是兼容层或 fallback，而是一条独立保留的历史产品线。
...
以下界面截图主要展示 `agent` 分支保留的论文工作流；`main` 复用同一前后端基础设施，但以建模指导、证据追踪和工作区导出为核心。
...
- 题目拆解、建模、模型审核、计算、结果复核和 Guidance 输出使用文件化阶段合同。
- 参数、数值、图表和最终结论必须绑定数据来源与结果证据，不能只给自然语言总结。
- 子问题可以分别标记为可解或阻断；缺少附录、约束或独立观测时不得伪造数值闭环。
- 工作区可以整体下载，包含 Guidance、注册表、Notebook、图表和诊断文件。
- 当前迭代正在以 Model IR 和通用算子执行图替换交通流、锚杆等整题模板。

## Agent 分支工作流

`agent` 分支的写作任务支持三种工作流模式：
